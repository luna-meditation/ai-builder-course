...
4. Только после проверки сервер вызывает атомарную функцию `upsert_telegram_profile`: новый Telegram ID создаёт ровно один профиль `student`, повторный вход обновляет только безопасные поля Telegram и `last_seen_at`.
5. Новый профиль не получает enrollment автоматически и открывает экран «Вы успешно зарегистрированы. Доступ к курсу пока не открыт».
6. Уникальное ограничение `profiles.telegram_user_id` и конфликтный путь функции защищают от дублей при одновременных запросах. Выполнять функцию может только `service_role`.
7. Роль администратора назначается только сервером по `ADMIN_TELEGRAM_IDS`; клиент не передаёт роль или статус доступа.
8. Сервер одним ответом возвращает роль, состояние доступа и нужный маршрут, затем выдаёт подписанную cookie-сессию на семь дней.
...
Источник истины для доступа: `profiles.is_blocked` и последняя запись `enrollments`. Отсутствие enrollment означает `no_access`, остальные состояния — `active`, `completed` или `revoked`. Кнопка «Проверить доступ» читает live-состояние отдельным запросом, не перезагружает Mini App и не использует системные alert.

Student routes используют короткий серверный кеш профиля/прогресса и 60-секундный кеш опубликованного контента. Кеш ученика точечно очищается после выдачи доступа, изменения прогресса, проверки или отправки задания. Открытие урока фиксируется после отображения страницы, поэтому Next.js prefetch не изменяет прогресс.
