...
function signedInitData(overrides: Record<string, string> = {}) {
  const params = new URLSearchParams({
    ...
    query_id: "AAEAAAE",
    user: JSON.stringify({ id: 123456789, first_name: "Тест", username: "tester" }),
    ...overrides,
  });
...
describe("Telegram initData", () => {
  it("проверяет подпись и возвращает подтверждённого пользователя", () => {
    expect(verifyTelegramInitData(signedInitData(), token)).toMatchObject({ id: 123456789, username: "tester" });
  });
