...
export function AdminNav({ firstName }: { firstName: string }) {
  ...
  async function enterPreview() {
    setPreviewBusy(true);
    try {
      const response = await fetch("/api/auth/preview", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ enabled: true }) });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error);
      router.replace("/course");
      router.refresh();
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Не удалось открыть предпросмотр");
  ...
  return <>
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 border-r border-[var(--border)] bg-[#0d0f14]/95 p-5 backdrop-blur-xl lg:block">
      ...
      <div className="absolute inset-x-5 bottom-5 rounded-2xl border border-[var(--border)] bg-white/[.025] p-4">
        ...
        <button type="button" onClick={() => void enterPreview()} disabled={previewBusy} className="focus-ring mt-3 flex items-center gap-1.5 text-xs font-semibold text-[#a89eff] disabled:opacity-60">{previewBusy ? <LoaderCircle className="size-3 animate-spin" /> : <Eye className="size-3" />} Посмотреть как ученик</button>
    ...
    <header className="safe-top mx-auto flex max-w-[1440px] items-center justify-between px-4 pt-2 lg:hidden"><Logo /><button type="button" onClick={() => void enterPreview()} disabled={previewBusy} className="focus-ring inline-flex min-h-10 items-center gap-2 rounded-xl border border-[var(--border)] bg-white/[.035] px-3 text-xs font-semibold text-[#b8b1ff] disabled:opacity-60">{previewBusy ? <LoaderCircle className="size-3.5 animate-spin" /> : <Eye className="size-3.5" />} Ученик</button></header>
