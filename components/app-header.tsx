...
export function AppHeader({ firstName, lastName, role }: { firstName: string; lastName?: string | null; role: string }) {
  ...
  return <header className="student-shell safe-top relative z-30 flex items-center justify-between py-4">
    ...
    <div className="glass flex items-center gap-1 rounded-2xl p-1.5 shadow-[0_10px_30px_rgba(0,0,0,.24)]">
      {role === "admin" && <Link href="/admin" className="focus-ring grid size-9 place-items-center rounded-xl text-[var(--muted)] transition hover:bg-white/[.07] hover:text-white" title="Админ-панель"><Shield className="size-4" /></Link>}
