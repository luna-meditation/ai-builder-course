...
import { CheckCircle2, File, Link2, MessageSquare, Paperclip, Send, Trash2, UploadCloud } from "lucide-react";
...
export function AssignmentForm({ lessonId, enrollmentId, description, submissions }: { lessonId: string; enrollmentId: string; description: string; submissions: Submission[] }) {
  const router = useRouter();
  const latest = submissions[0];
  const editable = !latest || latest.status === "draft" || latest.status === "revision_requested";
  ...
  const [saving, setSaving] = useState(false); const [submitting, setSubmitting] = useState(false); const [uploading, setUploading] = useState(false);
  ...
  useEffect(() => {
    if (!existingDraft) { const local = window.localStorage.getItem(storageKey); if (local) { try { reset(schema.parse(JSON.parse(local))); } catch { /* ignore invalid local data */ } } }
  ...
  }, [editable, existingDraft, reset, storageKey, watch]);
  ...
  async function submit() {
    if (!window.confirm("Отправить работу? После отправки изменить её можно будет только при возврате на доработку.")) return;
    setSubmitting(true);
    try { const draft = submissionId ? { id: submissionId } : await saveDraft(false); await persist("submit", draft.id); window.localStorage.removeItem(storageKey); toast.success("Работа отправлена"); router.refresh(); }
    catch (error) { toast.error(error instanceof Error ? error.message : "Не удалось отправить"); }
    finally { setSubmitting(false); }
  }

  const history = useMemo(() => submissions.filter((item) => item.status !== "draft"), [submissions]);
  return <div className="space-y-6">
    <section className="premium-panel overflow-hidden"><div className="border-b border-[var(--border)] p-5 sm:p-6"><p className="eyebrow">Финальный штрих</p><h2 className="mt-2 text-2xl font-bold tracking-[-.035em]">Покажи, что получилось</h2><p className="mt-3 text-sm leading-6 text-[var(--muted)]">{description}</p></div>
      {latest && !editable ? <div className="p-5 sm:p-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-semibold">Работа отправлена {formatDate(latest.submitted_at)}</p><p className="mt-1 text-xs text-[var(--muted)]">Попытка {latest.attempt_number}</p></div><StatusPill status={latest.status} /></div>{latest.status === "approved" && <div className="mt-5 flex items-center gap-3 rounded-2xl bg-[var(--success)]/[.08] p-4 text-sm text-[#8be8c2]"><CheckCircle2 className="size-5" /> Работа принята. Отличный результат!</div>}</div> : <div className="p-5 sm:p-6">
        ...
        <div><label htmlFor="assignment-text" className="text-sm font-semibold">Расскажи, что ты создал</label><textarea id="assignment-text" {...register("textContent")} rows={7} placeholder="Опиши результат, решения и то, с чем столкнулся…" className="focus-ring mt-2 w-full resize-y rounded-2xl border border-[var(--border)] bg-black/15 px-4 py-3 text-sm leading-6 outline-none transition-[border-color,background-color,box-shadow] placeholder:text-[#666b78] hover:border-white/[.15] focus:border-[#7667ff]/45 focus:bg-black/20" />{errors.textContent && <p className="mt-1 text-xs text-[var(--danger)]">{errors.textContent.message}</p>}</div>
        <div className="mt-4"><label htmlFor="assignment-url" className="flex items-center gap-2 text-sm font-semibold"><Link2 className="size-4" /> Публичная ссылка</label><input id="assignment-url" {...register("externalUrl")} placeholder="https://…" className="focus-ring mt-2 h-12 w-full rounded-2xl border border-[var(--border)] bg-black/15 px-4 text-sm outline-none transition-[border-color,background-color,box-shadow] placeholder:text-[#666b78] hover:border-white/[.15] focus:border-[#7667ff]/45 focus:bg-black/20" />{errors.externalUrl && <p className="mt-1 text-xs text-[var(--danger)]">{errors.externalUrl.message}</p>}</div>
        <div className="mt-5"><input ref={fileInput} type="file" multiple accept="image/jpeg,image/png,image/webp,application/pdf,.doc,.docx,video/mp4,video/quicktime" className="hidden" onChange={(event) => void onFiles(event.target.files)} /><button type="button" onClick={() => fileInput.current?.click()} disabled={uploading} className="focus-ring flex min-h-28 w-full flex-col items-center justify-center rounded-[22px] border border-dashed border-white/[.15] bg-white/[.02] text-sm font-semibold transition-[background-color,border-color,transform] hover:border-[#7667ff]/35 hover:bg-[#7667ff]/[.045] active:scale-[.99]"><UploadCloud className="mb-2 size-5 text-[#aaa1ff]" />{uploading ? "Загружаем…" : "Добавить файлы"}<span className="mt-1 text-[10px] font-normal text-[var(--muted)]">Изображения, PDF, DOCX или короткое видео · до 25 МБ</span></button></div>
        ...
        <div className="mt-6 grid gap-2 sm:grid-cols-[auto_1fr]"><Button variant="secondary" loading={saving} onClick={() => void saveDraft()}><Paperclip className="size-4" /> Сохранить черновик</Button><Button loading={submitting} disabled={uploading} onClick={() => void submit()}><Send className="size-4" /> {latest?.status === "revision_requested" ? "Отправить повторно" : "Отправить работу"}</Button></div>
      </div>}
    </section>
