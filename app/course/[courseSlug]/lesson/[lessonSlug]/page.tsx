...
export default async function LessonPage({ params }: { params: Promise<{ courseSlug: string; lessonSlug: string }> }) {
  ...
  return <main className="student-shell safe-top pb-16">
    ...
    <LessonVisit enrollmentId={data.enrollment.id} lessonId={data.lesson.id} disabled={Boolean(session.previewAsStudent) || !data.shouldMarkStarted} />
    ...
    {data.lesson.assignment_required && <section className="mt-12"><AssignmentForm lessonId={data.lesson.id} enrollmentId={data.enrollment.id} description={data.lesson.assignment_description} submissions={data.submissions} nextLessonHref={next && data.lesson.unlock_rule === "after_submission" ? `/course/${courseSlug}/lesson/${next.slug}` : undefined} preview={Boolean(session.previewAsStudent)} /></section>}
