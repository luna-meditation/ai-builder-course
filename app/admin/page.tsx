...
export default async function AdminDashboardPage() {
  ...
  return <><AdminHeader eyebrow="Панель управления" title="Сегодня в AI BUILDER" description="Реальные показатели курса и очередь действий — без демонстрационных метрик в production." /><section className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">{stats.map(({ label, value, icon: Icon, tone }) => <article key={label} className="card p-4"><span className={`grid size-10 place-items-center rounded-xl ${tone}`}><Icon className="size-4" /></span><strong className="mt-5 block text-2xl">{value}</strong><span className="mt-1 block text-xs text-[var(--muted)]">{label}</span></article>)}</section>
